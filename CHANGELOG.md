...
- **Discoverability** — `/catalog` now serves published creator songs in the exact shape the app's `catalogProvider` already maps to playable tracks; new `/metadata/search`, `/metadata/artist/:id`, `/metadata/track/:id`, and `/metadata/home` (New on DeeMusiq tracks + Top Artists This Year) surface creator content to the native plugin (YouTube source → playable) — the home screen now features creators instead of 404-retrying to YouTube.
